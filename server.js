...
class Entity {
    ...
    define(set) {
        ...
        if (set.GROW_OVER_TIME != null) { 
            this.settings.growsOverTime = set.GROW_OVER_TIME; 
        }
        ...
        if (set.INVISIBLE != null && set.INVISIBLE != []) this.invisible = [set.INVISIBLE[0], set.INVISIBLE[1]];
        if (set.DANGER != null) { 
...
//Yeet
    ...
    confinementToTheseEarthlyShackles() {
        ...
        let loc = { x: this.x, y: this.y, };
        if (room.isIn('ripp', loc)) {this.health.amount -= 1}
    }
    ...
    contemplationOfMortality() {
        ...
        if (this.isDead()) {
            ...
            if (killText === 'You have been kille') killText = 'You have died a stupid death, veery stupid death.';
        ...
        connect: (() => {
            ...
            const spawn = (() => {
                ...
                return (socket, name) => {
                    ...
                    body.sendMessage('Welcome to mog3s server! Have fun there and dont cheat.')
...
var maintainloop = (() => {
        ...
        let boss = (() => {
            ...
            return {
                prepareToSpawn: (classArray, number, nameClass, typeOfLocation = 'norm') => {
                    ...
                    i = 0;
                    if (n === 1) {
                        begin = 'A visitor is joining the area!';
                        arrival = names[0] + ' has arrived.'; 
                    } else {
                        begin = 'Visitors are coming to kill you.';
                        arrival = '';
            ...
            if (timer > 6000 && ran.dice(16000 - timer)) {
                ...
                switch (ran.chooseChance(40, 1)) {
                    ...
                    case 1: 
                        ...
                        break;
                    case 2:
                        choice = [[Class.azure_flare], 1, 'a', 'nest']
                        sockets.broadcast('The lights flickered.')
                        break;
                }
    ...
    let spawnCrasher = census => {
        if (ran.chance(1 -  0.5 * census.crasher / room.maxFood / room.nestFoodAmount)) {
            ...
            let type = (ran.dice(80)) ? ran.choose([Class.sentryGun, Class.sentrySwarm, Class.sentryTrap, Class.sentrylord, Class.azuresentry]) : Class.crasher;
            let o = new Entity(spot);
                o.define(type);
                o.team = -100;
        }
        ...
        return () => {
            ...
            if (c.GOLD == true) {
              ...
              if (Math.floor(Math.random() * 1000) == 500) {
                    let gold = new Entity(room.random());
                    gold.color = 10;
                    gold.define(Class.bot);
                    gold.define(Class.basic);
                    gold.name += ran.chooseBotName();
                    gold.refreshBodyAttributes();
                    gold.color = 10;
                    bots2.push(gold);
                    
                    bots2 = bots2.filter(e => { return !e.isDead(); });
                    gold.skill.score += 2E5;
                    gold.skill.maintain();
              }
             }
            }
...
let server = http.createServer((req, res) => {
  ...
  switch (pathname) {
    case '/':
      res.writeHead(200)
      res.end(`<!DOCTYPE html><title>Mog3s server</title><style>* {font-family: Tahoma};</style><style>body {margin: 16px; background-color: #f0f0f0}</style><style>button {border-radius: 4px;border: 1.5px ridge #c7c7c7;}</style><h3>Nice try. But no. Go away.</h3>`)
    break
    case '/mockups.json':
